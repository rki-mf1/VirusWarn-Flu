...
if(params$metadata != ''){
  metadata_end <- substr(params$metadata, nchar(params$metadata) - 2, nchar(params$metadata))
  if (metadata_end == "lsx"){
    metadata <- read_xlsx(params$metadata)
  } else if (metadata_end == "xls"){
    metadata <- read_xls(params$metadata)
  }
}
```
...
#  cat("No metadata file with collection dates was given, so no heatmap was generated.")
  ...
  df_mutations_grouped <- df_mutations_filtered %>% 
    ...
    summarize(freq = n()) %>%
    mutate(perc = round(freq/length(df_names)*100, digits = 2)) %>% 
    ungroup() 
  ...
  barplot <- plot_ly(
    ...
    layout(
      xaxis=list(nticks=length(week_tick), title='<b>Week (Collection Date)</b>', showgrid=FALSE),
      yaxis=list(title="<b>Number <br> of sequences</b>", showgrid=FALSE)
    )
