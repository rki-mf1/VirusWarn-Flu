...
Sequences are sent to a category (Red, Orange, Pink, Grey) according to their mutation profile. FluWarnSystem counts the amount of mutations in 3 categories:
...
fixed_table <- filter(fixed_table, Season == params$season)
...
alert_colors <- c(grey = "lightgrey", pink= "deeppink", orange = "orange", red = "red")
...
if (params$metadata == ""){
  ...
  if (nrow(df_bar) != 5){
    all_alert_levels <- c('grey', 'pink', 'orange', 'red')
  ...
  order_levels <- c('grey', 'pink', 'orange', 'red')
...
for (i in pink_table["ID"]) {
  new <- str_remove_all(i, "%space%")
  pink_table["ID"][pink_table["ID"] == i] <- new
}

names(pink_table)[2] <- "Mutations"
names(pink_table)[3] <- "Deletions"
names(pink_table)[4] <- "Insertions"

pink_table_copy <- pink_table

for (i in pink_table["Mutations"]){
  new <- str_split(i, ",")
  pink_table["Mutations"][pink_table["Mutations"] == i] <- new
}
...
if (nrow(pink_table) == 0){
  ...
  for (i in pink_table_copy["Mutations"]){
    new <- gsub(",([[:alpha:]])", ", \\1", i)
    pink_table_copy["Mutations"][pink_table_copy["Mutations"] == i] <- new
  }
  
  subtype <- vector(mode = "character", length = length(pink_table_copy$ID))
  seg <- vector(mode = "character", length = length(pink_table_copy$ID))
  
  moc <- vector(mode = "character", length = length(pink_table_copy$ID))
  roi <- vector(mode = "character", length = length(pink_table_copy$ID))
  pm <- vector(mode = "character", length = length(pink_table_copy$ID))
    
  pink_table_copy <- cbind(subtype, seg, pink_table_copy, moc, roi, pm)
    
  names(pink_table_copy)[1] <- "Subtype"
  names(pink_table_copy)[2] <- "Segment"
  
  names(pink_table_copy)[4] <- "Substitutions"
  
  names(pink_table_copy)[7] <- "MOC"
  names(pink_table_copy)[8] <- "ROI"
  names(pink_table_copy)[9] <- "PM"
  
  for (j in 1:length(pink_table_copy$ID)){
    pink_table_copy$Subtype[j] <- str_split_i(pink_table_copy$ID[j], "\\|", -1)
    pink_table_copy$Segment[j] <- str_split_i(pink_table_copy$ID[j], "\\|", 2)
  }
  
  for (k in 1:length(pink_table_copy$ID)){
    temp <- subset(annot_table, ID == pink_table_copy$ID[k])
    pink_table_copy$MOC[k] <- nrow(subset(temp, type=="MutationOfConcern")) 
    pink_table_copy$ROI[k] <- nrow(subset(temp, type=="RegionOfInterest")) 
    pink_table_copy$PM[k] <- nrow(subset(temp, type=="NotAnnotated")) 
  }
  
  col_order <- c("Subtype", "Segment", "ID", "MOC", "ROI", "PM", "Substitutions", "Deletions", "Insertions")
  pink_table_copy <- pink_table_copy[, col_order]
...
```{r pink heatmap, out.width="1000px", echo=FALSE, message=FALSE, warning=FALSE}
if (!(nrow(pink_table) == 0)){
  names <- vector(mode = "character", length = length(pink_table$ID))
  df_names <- cbind(pink_table, names)
  names(df_names)[5] <- "Isolate_Name"
  for (j in 1:length(df_names$ID)){
    df_names$Isolate_Name[j] <- trimws(str_split_i(df_names$ID[j], "\\|", 3))
  }
