...
samples <- samples %>%
  filter(!ID %in% 'ROI_ERROR_CATCH')
```

```{r functions, include=FALSE}
create_button_list <- function(groups) {
  button_list <- list()
  for (i in seq_along(groups)) {
    button <- list(
      label = groups[i],
      method = "update",
      args = list(list(visible = rep(FALSE, length(groups))),
                  list(title = groups[i])),
      visible = TRUE
    )
...
```{r red clades, out.width="1400px", echo=FALSE, message=FALSE, warning=FALSE}
clade <- c()
for(i in red_table$ID){
...
bool_list <- list()
for (i in 1:(clade_len)){
  new_list <- rep(TRUE, (clade_len))
  #new_list[i] <- TRUE
  bool_list[[i]] <- new_list
}

button_list <- list(
  label = "All sequences",
  method = "update",
  args = list(list(visible=TRUE), list(title="All sequences"))
)
for (i in 1:clade_len){
  list_clade <- list(
    label = clade_unique[[i]],
    method = "update",
    args = list(list(visible=bool_list[[i]]), list(title=clade_unique[[i]]))
  )
  button_list <- list(button_list, list_clade)
}
...
if (nrow(red_table) == 0){
  ...
  for (df in df_list){
    ...
    data_temp <- data.frame(x = seq(1, 566), y = mut_freq, list = mut_list_unique, group = df$group[1])
  ...
  p <- ggplotly(ggplot, tooltip = c("text"))
...
if (nrow(red_table) == 0){
  ...
  plot_ly(
    ...
    layout(
      plot_bgcolor="#F8F8F8", 
      xaxis=list(title='<b>Week (Collection Date)</b>', showgrid=FALSE),
      yaxis=list(nticks=length(aa_tick), title="<b>Mutation</b>", showgrid=FALSE, categoryorder="trace"),
...
```{r orange clades, out.width="1400px", echo=FALSE, message=FALSE, warning=FALSE}
clade <- c()
for(i in orange_table$ID){
  new_clade <- variant_table[variant_table$ID == i, "clade"]
  clade <- c(clade, new_clade[[1]])
...
```{r yellow clades, out.width="1400px", echo=FALSE, message=FALSE, warning=FALSE}
clade <- c()
for(i in yellow_table$ID){
  new_clade <- variant_table[variant_table$ID == i, "clade"]
  clade <- c(clade, new_clade[[1]])
...
```{r grey clades, out.width="1400px", echo=FALSE, message=FALSE, warning=FALSE}
clade <- c()
for(i in grey_table$ID){
  new_clade <- variant_table[variant_table$ID == i, "clade"]
  clade <- c(clade, new_clade[[1]])
